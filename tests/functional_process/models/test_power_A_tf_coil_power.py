"""Harness cases for `functional_process/models/power_A_tf_coil_power.py`.

Audit record: `functional_process/_audit/units/models/power_A_tf_coil_power.md`. No legacy points
exist for either function in `tests/unit/models/test_power.py` (`tfpwr`/`tfpwcall`/
`tfcpwr` have no automatically-generated unit test there, unlike `cryo`/`acpow`/
`plant_electric_production` -- see `power_B_thermal_cryo.md`/`power_C_electric_
production.md` for those) -- fuzz-only, same situation `build.md` documented for
`st_build`.
"""
...
class TestTfPowerResistive(Tier1Contract):
    ...
    audit_record = "models/power_A_tf_coil_power.md"
...
def _tf_power_superconducting_samples():
    ...
    see `power_A_tf_coil_power.md`'s open questions) that the port agrees with
