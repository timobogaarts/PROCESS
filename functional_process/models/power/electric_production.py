...
from functional_process.power.electric_production import (
    calculate_acpow,
...
# Only `jnp` and `calculate_acpow` are actually unused below (the `Acpow` base class
# only names the family in its docstring; the two leaf arms call
# `calculate_acpow_line`/`calculate_acpow_motor_generator_flywheel` instead) -- kept so
# this module keeps resolving every public name it resolved before step 2 of
# `_audit/formulas_split.md` moved the pure functions out (the split's invariant; see
# `functional_process.power.electric_production` for the real uses). `__all__` has to
# list every other public name too, not just those two: ruff's docstring rules treat
# `__all__` membership as the definition of "public" once it is present, so a partial
# list would silently turn off D101/D102 on the classes below instead of leaving them
# checked exactly as before this file had an `__all__` at all.
__all__ = [
    "Acpow",
    "AcpowLine",
