...
def calculate_divertor_surface_and_plate_mass_single_null(
    ...
    **writes** it at `:353`, so a single node covering the whole method would read what
    it owns -- a read-your-own-write hard error in a declared graph, and in PROCESS a
    silent one-pass-stale read that only the outer Gauss-Seidel loop repairs.
...
def calculate_component_masses(
    ...
    **Deviation: `.fwbs.breeder_f` is not owned by this node.** PROCESS clamps it in
    ...
    is a self-loop in a declared graph. It dissolves rather than needing a
    `FixedPointFunction`, for two measured reasons: the clamp is idempotent, and
    `.fwbs.breeder_f`'s **only reader
