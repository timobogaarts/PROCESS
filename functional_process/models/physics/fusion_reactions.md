...
| `.physics.profile_x` (**newly minted**) | read | explicit-arg | `teprofile.profile_x` and `neprofile.profile_x` are two distinct array objects but numerically identical — both are built by `profiles.py`'s `Profile.normalise_profile_x()` acting on `np.arange(n_plasma_profile_elements)`, with no per-instance randomness or divergence, so this port takes **one** shared grid argument rather than two. Verified, not assumed: `plasma_profiles.py`'s own test harness stub (`test_plasma_profiles.py`'s `_plasma_profile`) already sets `neprofile.profile_x` and `teprofile.profile_x` to the identical array for the same reason. No existing `VarPath` covers `profile_x` — `plasma_profiles.md` explicitly left it un-minted (only its `profile_y` counterparts were minted, because the only node that port shipped, `ProfileFactors`, never needed the grid itself). Minting it here as `.physics.profile_x`, in `.physics` for the same reason `plasma_profiles.md` minted the `profile_y` pair there — every other profile-shaped array in this file already lives in that namespace. |
