...
Two switches touch `bldgs_sizes`:
- `is_neutral_beam` -- **no longer a static field.** `.current_drive.i_hcd_primary` has
  12 values but `bldgs_sizes` reads it to decide one bit (whether the method is
  neutral-beam-shaped), so the cottax node is two occupants
  (`BldgsSizesNeutralBeam`/`BldgsSizesOtherHcd`, in `cottax/models/buildings/
  buildings.py`), each calling one of `calculate_bldgs_sizes_neutral_beam`/
  `calculate_bldgs_sizes_other_hcd` below -- thin wrappers baking `is_neutral_beam` to a
  literal, so `calculate_bldgs_sizes` itself is untouched and `is_neutral_beam` stays an
  ordinary parameter on it. Which `i_hcd_primary` values reach which arm is decided at
  graph-construction time (`indat.py`'s `_bldgs_sizing_arm`), via the same
  `CurrentDriveModel(...).method == CurrentDriveMethodType.NEUTRAL_BEAM` lookup the old
  static field used to do per-trace.
- `i_tf_sup` (`bldgs_sizes`'s centre-post branch only) -- an ordinary traced argument
  ...
  Unaffected by the above: it is not a model-choice switch in cottax's sense (both
  values are ordinary numeric branches of one differentiable formula), so it stays a
  plain read on every occupant.
...
def calculate_bldgs_sizes_neutral_beam(
    r_pf_coil_outer_max,
    r_cryostat_inboard,
    tf_radial_dim,
    bioshld_thk,
    ...
    """`calculate_bldgs_sizes`, `is_neutral_beam == True` baked --
    the `i_hcd_primary`-selected arm whose primary HCD method is neutral-beam-shaped
    (`ITER_NEUTRAL_BEAM`/`CULHAM_NEUTRAL_BEAM`). Which `i_hcd_primary` values reach
    this arm is decided at graph-construction time (`indat.py`'s `_bldgs_sizing_arm`),
    not by this function -- see the module docstring's "two switches" note.
    ...
    :
        `(reactor_hall_l, reactor_hall_w, reactor_hall_h, a_plant_floor_effective,
        volnucb)`, this arm's outputs, in that order.
    """
    return calculate_bldgs_sizes(
        r_pf_coil_outer_max=r_pf_coil_outer_max,
        r_cryostat_inboard=r_cryostat_inboard,
        tf_radial_dim=tf_radial_dim,
        bioshld_thk=bioshld_thk,
        reactor_clrnc=reactor_clrnc,
        transp_clrnc=transp_clrnc,
        crane_clrnc_h=crane_clrnc_h,
        cryostat_clrnc=cryostat_clrnc,
        ground_clrnc=ground_clrnc,
        crane_arm_h=crane_arm_h,
        tf_vertical_dim=tf_vertical_dim,
        nbi_sys_l=nbi_sys_l,
        nbi_sys_w=nbi_sys_w,
        hcd_building_l=hcd_building_l,
        hcd_building_w=hcd_building_w,
        hcd_building_h=hcd_building_h,
        fc_building_l=fc_building_l,
        fc_building_w=fc_building_w,
        reactor_wall_thk=reactor_wall_thk,
        reactor_roof_thk=reactor_roof_thk,
        reactor_fndtn_thk=reactor_fndtn_thk,
        life_plant=life_plant,
        z_tf_inside_half=z_tf_inside_half,
        dr_tf_inboard=dr_tf_inboard,
        dr_tf_shld_gap=dr_tf_shld_gap,
        dz_shld_thermal=dz_shld_thermal,
        ...
        hot_sepdist=hot_sepdist,
        qnty_sfty_fac=qnty_sfty_fac,
        ...
        heat_sink_w=heat_sink_w,
        heat_sink_h=heat_sink_h,
        aux_build_l=aux_build_l,
        aux_build_w=aux_build_w,
        aux_build_h=aux_build_h,
        magnet_trains_l=magnet_trains_l,
        magnet_trains_w=magnet_trains_w,
        magnet_trains_h=magnet_trains_h,
        magnet_pulse_l=magnet_pulse_l,
        magnet_pulse_w=magnet_pulse_w,
        magnet_pulse_h=magnet_pulse_h,
        control_buildings_l=control_buildings_l,
        control_buildings_w=control_buildings_w,
        control_buildings_h=control_buildings_h,
        warm_shop_l=warm_shop_l,
        warm_shop_w=warm_shop_w,
        warm_shop_h=warm_shop_h,
        workshop_l=workshop_l,
        workshop_w=workshop_w,
        workshop_h=workshop_h,
        robotics_l=robotics_l,
        robotics_w=robotics_w,
        robotics_h=robotics_h,
        maint_cont_l=maint_cont_l,
        maint_cont_w=maint_cont_w,
        maint_cont_h=maint_cont_h,
        cryomag_l=cryomag_l,
        cryomag_w=cryomag_w,
        cryomag_h=cryomag_h,
        cryostore_l=cryostore_l,
        cryostore_w=cryostore_w,
        cryostore_h=cryostore_h,
        auxcool_l=auxcool_l,
        auxcool_w=auxcool_w,
        auxcool_h=auxcool_h,
        elecdist_l=elecdist_l,
        elecdist_w=elecdist_w,
        elecdist_h=elecdist_h,
        elecload_l=elecload_l,
        elecload_w=elecload_w,
        elecload_h=elecload_h,
        elecstore_l=elecstore_l,
        elecstore_w=elecstore_w,
        elecstore_h=elecstore_h,
        turbine_hall_l=turbine_hall_l,
        turbine_hall_w=turbine_hall_w,
        turbine_hall_h=turbine_hall_h,
        ilw_smelter_l=ilw_smelter_l,
        ilw_smelter_w=ilw_smelter_w,
        ilw_smelter_h=ilw_smelter_h,
        ilw_storage_l=ilw_storage_l,
        ilw_storage_w=ilw_storage_w,
        ilw_storage_h=ilw_storage_h,
        llw_storage_l=llw_storage_l,
        llw_storage_w=llw_storage_w,
        llw_storage_h=llw_storage_h,
        hw_storage_l=hw_storage_l,
        hw_storage_w=hw_storage_w,
        hw_storage_h=hw_storage_h,
        tw_storage_l=tw_storage_l,
        tw_storage_w=tw_storage_w,
        tw_storage_h=tw_storage_h,
        gas_buildings_l=gas_buildings_l,
        gas_buildings_w=gas_buildings_w,
        gas_buildings_h=gas_buildings_h,
        water_buildings_l=water_buildings_l,
        water_buildings_w=water_buildings_w,
        water_buildings_h=water_buildings_h,
        sec_buildings_l=sec_buildings_l,
        sec_buildings_w=sec_buildings_w,
        sec_buildings_h=sec_buildings_h,
        staff_buildings_area=staff_buildings_area,
        staff_buildings_h=staff_buildings_h,
        is_neutral_beam=True,
    )


def calculate_bldgs_sizes_other_hcd(
    r_pf_coil_outer_max,
    r_cryostat_inboard,
    tf_radial_dim,
    bioshld_thk,
    ...
    """`calculate_bldgs_sizes`, `is_neutral_beam == False` baked --
    every other `i_hcd_primary` value. Same reasoning as
    `calculate_bldgs_sizes_neutral_beam`, opposite bit.
