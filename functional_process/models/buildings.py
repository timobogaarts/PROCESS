...
class TfCoilEnvelope(ExplicitFunction):
    ...
    def __call__(
        ...
        r_tf_inboard_mid=Input(lambda s: s.build.r_tf_inboard_mid),
        dr_tf_inboard=Input(lambda s: s.build.dr_tf_inboard),
        z_tf_inside_half=Input(lambda s: s.build.z_tf_inside_half),
...
class Bldgs(ExplicitFunction):
    ...
    def __call__(
        self,
        pfr=Input(lambda s: s.pf_coil.r_pf_coil_outer_max),
        pfm=Input(lambda s: s.pf_coil.m_pf_coil_max),
        tfro=Input(lambda s: s.buildings.tfro),
        tfri=Input(lambda s: s.buildings.tfri),
        tfh=Input(lambda s: s.buildings.tf_vertical_dim),
        tfm=Input(lambda s: s.buildings.tfmtn),
        n_tf_coils=Input(lambda s: s.tfcoil.n_tf_coils),
        shro=Input(lambda s: s.build.r_shld_outboard_outer),
        shri=Input(lambda s: s.build.r_shld_inboard_inner),
        z_tf_inside_half=Input(lambda s: s.build.z_tf_inside_half),
        dz_shld_vv_gap=Input(lambda s: s.build.dz_shld_vv_gap),
        dz_vv_upper=Input(lambda s: s.build.dz_vv_upper),
        dz_vv_lower=Input(lambda s: s.build.dz_vv_lower),
        shm=Input(lambda s: s.fwbs.whtshld),
        crr=Input(lambda s: s.fwbs.r_cryostat_inboard),
        helpow=Input(lambda s: s.heat_transport.helpow),
        rxcl=Input(lambda s: s.buildings.rxcl),
        trcl=Input(lambda s: s.buildings.trcl),
        row=Input(lambda s: s.buildings.row),
        wgt=Input(lambda s: s.buildings.wgt),
        shmf=Input(lambda s: s.buildings.shmf),
        clh2=Input(lambda s: s.buildings.clh2),
        dz_tf_cryostat=Input(lambda s: s.buildings.dz_tf_cryostat),
        stcl=Input(lambda s: s.buildings.stcl),
        rbvfac=Input(lambda s: s.buildings.rbvfac),
        rbwt=Input(lambda s: s.buildings.rbwt),
        rbrt=Input(lambda s: s.buildings.rbrt),
        fndt=Input(lambda s: s.buildings.fndt),
        hcwt=Input(lambda s: s.buildings.hcwt),
        hccl=Input(lambda s: s.buildings.hccl),
        wgt2=Input(lambda s: s.buildings.wgt2),
        mbvfac=Input(lambda s: s.buildings.mbvfac),
        wsvfac=Input(lambda s: s.buildings.wsvfac),
        tfcbv=Input(lambda s: s.buildings.tfcbv),
        pfbldgm3=Input(lambda s: s.buildings.pfbldgm3),
        esbldgm3=Input(lambda s: s.buildings.esbldgm3),
        pibv=Input(lambda s: s.buildings.pibv),
        triv=Input(lambda s: s.buildings.triv),
        conv=Input(lambda s: s.buildings.conv),
        admv=Input(lambda s: s.buildings.admv),
        shov=Input(lambda s: s.buildings.shov),
    ):
...
class BldgsSizes(ExplicitFunction):
    ...
    `i_hcd_primary` is a static field (not an `Input`) -- see module docstring.
    """
    ...
    def __call__(
        self,
        r_pf_coil_outer_max=Input(lambda s: s.pf_coil.r_pf_coil_outer_max),
        r_cryostat_inboard=Input(lambda s: s.fwbs.r_cryostat_inboard),
        tf_radial_dim=Input(lambda s: s.buildings.tf_radial_dim),
        bioshld_thk=Input(lambda s: s.buildings.bioshld_thk),
        reactor_clrnc=Input(lambda s: s.buildings.reactor_clrnc),
        transp_clrnc=Input(lambda s: s.buildings.transp_clrnc),
        crane_clrnc_h=Input(lambda s: s.buildings.crane_clrnc_h),
        cryostat_clrnc=Input(lambda s: s.buildings.cryostat_clrnc),
        ground_clrnc=Input(lambda s: s.buildings.ground_clrnc),
        crane_arm_h=Input(lambda s: s.buildings.crane_arm_h),
        tf_vertical_dim=Input(lambda s: s.buildings.tf_vertical_dim),
        nbi_sys_l=Input(lambda s: s.buildings.nbi_sys_l),
        nbi_sys_w=Input(lambda s: s.buildings.nbi_sys_w),
        hcd_building_l=Input(lambda s: s.buildings.hcd_building_l),
        hcd_building_w=Input(lambda s: s.buildings.hcd_building_w),
        hcd_building_h=Input(lambda s: s.buildings.hcd_building_h),
        fc_building_l=Input(lambda s: s.buildings.fc_building_l),
        fc_building_w=Input(lambda s: s.buildings.fc_building_w),
        reactor_wall_thk=Input(lambda s: s.buildings.reactor_wall_thk),
        reactor_roof_thk=Input(lambda s: s.buildings.reactor_roof_thk),
        reactor_fndtn_thk=Input(lambda s: s.buildings.reactor_fndtn_thk),
        life_plant=Input(lambda s: s.costs.life_plant),
        z_tf_inside_half=Input(lambda s: s.build.z_tf_inside_half),
        dr_tf_inboard=Input(lambda s: s.build.dr_tf_inboard),
        dr_tf_shld_gap=Input(lambda s: s.build.dr_tf_shld_gap),
        dz_shld_thermal=Input(lambda s: s.build.dz_shld_thermal),
        dz_shld_vv_gap=Input(lambda s: s.build.dz_shld_vv_gap),
        ...
        n_tf_coils=Input(lambda s: s.tfcoil.n_tf_coils),
        hot_sepdist=Input(lambda s: s.buildings.hot_sepdist),
        qnty_sfty_fac=Input(lambda s: s.buildings.qnty_sfty_fac),
        ...
        dz_divertor=Input(lambda s: s.divertor.dz_divertor),
        cplife=Input(lambda s: s.costs.cplife),
        i_tf_sup=Input(lambda s: s.tfcoil.i_tf_sup),
        r_cp_top=Input(lambda s: s.build.r_cp_top),
        hotcell_h=Input(lambda s: s.buildings.hotcell_h),
        chemlab_l=Input(lambda s: s.buildings.chemlab_l),
        chemlab_w=Input(lambda s: s.buildings.chemlab_w),
        chemlab_h=Input(lambda s: s.buildings.chemlab_h),
        heat_sink_l=Input(lambda s: s.buildings.heat_sink_l),
        heat_sink_w=Input(lambda s: s.buildings.heat_sink_w),
        heat_sink_h=Input(lambda s: s.buildings.heat_sink_h),
        aux_build_l=Input(lambda s: s.buildings.aux_build_l),
        aux_build_w=Input(lambda s: s.buildings.aux_build_w),
        aux_build_h=Input(lambda s: s.buildings.aux_build_h),
        magnet_trains_l=Input(lambda s: s.buildings.magnet_trains_l),
        magnet_trains_w=Input(lambda s: s.buildings.magnet_trains_w),
        magnet_trains_h=Input(lambda s: s.buildings.magnet_trains_h),
        magnet_pulse_l=Input(lambda s: s.buildings.magnet_pulse_l),
        magnet_pulse_w=Input(lambda s: s.buildings.magnet_pulse_w),
        magnet_pulse_h=Input(lambda s: s.buildings.magnet_pulse_h),
        control_buildings_l=Input(lambda s: s.buildings.control_buildings_l),
        control_buildings_w=Input(lambda s: s.buildings.control_buildings_w),
        control_buildings_h=Input(lambda s: s.buildings.control_buildings_h),
        warm_shop_l=Input(lambda s: s.buildings.warm_shop_l),
        warm_shop_w=Input(lambda s: s.buildings.warm_shop_w),
        warm_shop_h=Input(lambda s: s.buildings.warm_shop_h),
        workshop_l=Input(lambda s: s.buildings.workshop_l),
        workshop_w=Input(lambda s: s.buildings.workshop_w),
        workshop_h=Input(lambda s: s.buildings.workshop_h),
        robotics_l=Input(lambda s: s.buildings.robotics_l),
        robotics_w=Input(lambda s: s.buildings.robotics_w),
        robotics_h=Input(lambda s: s.buildings.robotics_h),
        maint_cont_l=Input(lambda s: s.buildings.maint_cont_l),
        maint_cont_w=Input(lambda s: s.buildings.maint_cont_w),
        maint_cont_h=Input(lambda s: s.buildings.maint_cont_h),
        cryomag_l=Input(lambda s: s.buildings.cryomag_l),
        cryomag_w=Input(lambda s: s.buildings.cryomag_w),
        cryomag_h=Input(lambda s: s.buildings.cryomag_h),
        cryostore_l=Input(lambda s: s.buildings.cryostore_l),
        cryostore_w=Input(lambda s: s.buildings.cryostore_w),
        cryostore_h=Input(lambda s: s.buildings.cryostore_h),
        auxcool_l=Input(lambda s: s.buildings.auxcool_l),
        auxcool_w=Input(lambda s: s.buildings.auxcool_w),
        auxcool_h=Input(lambda s: s.buildings.auxcool_h),
        elecdist_l=Input(lambda s: s.buildings.elecdist_l),
        elecdist_w=Input(lambda s: s.buildings.elecdist_w),
        elecdist_h=Input(lambda s: s.buildings.elecdist_h),
        elecload_l=Input(lambda s: s.buildings.elecload_l),
        elecload_w=Input(lambda s: s.buildings.elecload_w),
        elecload_h=Input(lambda s: s.buildings.elecload_h),
        elecstore_l=Input(lambda s: s.buildings.elecstore_l),
        elecstore_w=Input(lambda s: s.buildings.elecstore_w),
        elecstore_h=Input(lambda s: s.buildings.elecstore_h),
        turbine_hall_l=Input(lambda s: s.buildings.turbine_hall_l),
        turbine_hall_w=Input(lambda s: s.buildings.turbine_hall_w),
        turbine_hall_h=Input(lambda s: s.buildings.turbine_hall_h),
        ilw_smelter_l=Input(lambda s: s.buildings.ilw_smelter_l),
        ilw_smelter_w=Input(lambda s: s.buildings.ilw_smelter_w),
        ilw_smelter_h=Input(lambda s: s.buildings.ilw_smelter_h),
        ilw_storage_l=Input(lambda s: s.buildings.ilw_storage_l),
        ilw_storage_w=Input(lambda s: s.buildings.ilw_storage_w),
        ilw_storage_h=Input(lambda s: s.buildings.ilw_storage_h),
        llw_storage_l=Input(lambda s: s.buildings.llw_storage_l),
        llw_storage_w=Input(lambda s: s.buildings.llw_storage_w),
        llw_storage_h=Input(lambda s: s.buildings.llw_storage_h),
        hw_storage_l=Input(lambda s: s.buildings.hw_storage_l),
        hw_storage_w=Input(lambda s: s.buildings.hw_storage_w),
        hw_storage_h=Input(lambda s: s.buildings.hw_storage_h),
        tw_storage_l=Input(lambda s: s.buildings.tw_storage_l),
        tw_storage_w=Input(lambda s: s.buildings.tw_storage_w),
        tw_storage_h=Input(lambda s: s.buildings.tw_storage_h),
        gas_buildings_l=Input(lambda s: s.buildings.gas_buildings_l),
        gas_buildings_w=Input(lambda s: s.buildings.gas_buildings_w),
        gas_buildings_h=Input(lambda s: s.buildings.gas_buildings_h),
        water_buildings_l=Input(lambda s: s.buildings.water_buildings_l),
        water_buildings_w=Input(lambda s: s.buildings.water_buildings_w),
        water_buildings_h=Input(lambda s: s.buildings.water_buildings_h),
        sec_buildings_l=Input(lambda s: s.buildings.sec_buildings_l),
        sec_buildings_w=Input(lambda s: s.buildings.sec_buildings_w),
        sec_buildings_h=Input(lambda s: s.buildings.sec_buildings_h),
        staff_buildings_area=Input(lambda s: s.buildings.staff_buildings_area),
        staff_buildings_h=Input(lambda s: s.buildings.staff_buildings_h),
    ):
        is_neutral_beam = (
