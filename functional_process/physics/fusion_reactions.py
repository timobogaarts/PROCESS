"""Pure physics functions extracted from `models/physics/fusion_reactions.py`.

See that file for the declarations wiring these into the graph, and
`_audit/units/models/physics/fusion_reactions.md` for the audit record. No
...
def calculate_deuterium_branching_trit(ion_temperature):
    ...
    Ports `.deuterium_branching()`. No graph node of its own -- see the audit record's
    node-scope section: its only consumer within scope is `FusionRates`, which calls
...
def calculate_fusion_rates(
    ...
    `.set_physics_variables()` -- see the audit record's node-scope section for why
