...
def _missing_producers() -> tuple[VarPath, ...]:
    """`unproduced_but_computed` for `MISSING_PRODUCERS_INPUT_FILE`'s machine.

    The one composition `MISSING_PRODUCERS_PIN`'s own docstring promised as
    `--missing --write` and that nothing implemented until 2026-08-30, so the four
    producers landed that day had to have the pin regenerated by hand -- which is
    exactly the transcription risk `write_pin` exists to remove.

    Assembles what `test_no_new_boundary_input_is_something_process_computes` assembles,
    in the same order: PROCESS's measured write set, the run's iteration variables (which
    are boundary inputs *on purpose*), and the driven graph.
    """
    ...
    machine = machine_from_indat(MISSING_PRODUCERS_INPUT_FILE)
    design = {
        iteration_variable_path(i)
        for i in reference_run(MISSING_PRODUCERS_INPUT_FILE).ixc
    }
    return unproduced_but_computed(
        driven_graph(graph_for(machine)),
        computed_by_process(MISSING_PRODUCERS_INPUT_FILE),
        design,
    )


def _main_missing(argv: list[str]) -> int:
    """`--missing [--write]`: the missing-producer list, printed or pinned."""
    found = _missing_producers()
