...
from functional_process.tests._harness.boundary import (
    DIVISION_BY_ZERO_AT_BOUNDARY,
    registered_reason,
)
from functional_process.tests._harness import sample_store
from functional_process.tests._harness.finite_difference import (
    PROCESS_EPSFCN,
    ZeroPerturbationError,
    fd_gradient_with_error,
)
from functional_process.tests._harness.sampling import fuzz_samples
from functional_process.tests._harness.tolerance import (
    MACHINE_PRECISION,
    DeclaredDeviation,
    Tolerance,
)
...
class PortContract:
    ...
    def __init_subclass__(cls, **kwargs):
        ...
        # list for every reader -- conftest's parametrisation and the cases
...
class Tier1Contract(PortContract):
    ...
    def test_declared_deviation_is_documented(self, package_root):
        ...
        assert (package_root / cited).is_file(), (
            f"{type(self).__name__}'s declared deviation cites {deviation.record}, "
            f"which is not a file under {package_root}"
        )
    ...
    def test_gradient_finite_at_zero(self):
        ...
        assert not failures, "\n".join([
            ...
            "the `x ** p`, `0 < p < 1`, trap. Fix it with `models/safe_math.py`'s `safe_pow` "
