...
from functional_process.cottax._harness.registry import (
    parse_frontmatter,
    parse_unit_registry,
)
...
# under `functional_process/tests/`, mirroring it too — so `__file__` points near neither
...
def _case_for(record):
    ...
    `functional_process/tests/` and prefixing the stem with `test_`.
    """
