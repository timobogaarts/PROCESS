...
## Update: port pass (see `plasma_physics.py` / `test_plasma_physics.py`)
...
`pure_formulas.py`, `composition.py`, `confinement_time.py`,
...
| `self.beta.fast_alpha_beta(...)` (confirmed: `self.beta` *is* `self.plasma_beta`, `PlasmaBeta.fast_alpha_beta`, per open question 2 below) | `FastAlphaBeta` | `models/physics/pure_formulas.py` |
| `rether(...)` | `IonElectronEquilibration` | `models/physics/pure_formulas.py` |
| `physics_funcs.calculate_radiation_powers(...)` | `PlasmaRadiationPowers` | `models/physics/radiation_power.py` |
| `self.physics.calaculate_stored_thermal_energy(...)` (×2, electron and ion) | `ElectronThermalEnergy`, `IonThermalEnergy` | `models/physics/pure_formulas.py` |
...
| `self.physics.calculate_total_plasma_heating_power(...)` | `TotalPlasmaHeatingPower` | `models/physics/pure_formulas.py` |
| `self.physics.exhaust.calculate_radiation_fraction(...)` | `RadiationFraction` | `models/physics/exhaust.py` |
| `self.physics.phyaux(...)` | `AuxiliaryPhysicsQuantities` | `models/physics/pure_formulas.py` |
...
- **`self.physics.plasma_composition()`** — `composition.md`'s own finding:
...
3. **`beta_fast_alpha`/`beta_beam`, resolved: both are Shape A (ordinary acyclic
   ...
   `.physics.first_call` self-reference (`composition.md`'s finding,
...
Eight nodes, all in `plasma_physics.py`, all `ExplicitFunction`s, no internal
...
**Tests**: `test_plasma_physics.py`, all `Tier1Contract`. None of the eight
...
`structure.md`'s `calculate_intercoil_mass_scaling_reference` was in.
`~/miniconda3/envs/process_port/bin/python -m pytest
functional_process/models/stellarator/test_plasma_physics.py -q --fp-gradients`:
...
`pure_formulas.fast_alpha_beta` is the standard clamped-`sqrt` autodiff trap,
