...
**Ported (3 units).** `electric_production.py` /
`test_electric_production.py`: `calculate_acpow`, `power_profiles_over_time`,
...
`tf_coil_power.md`/`thermal_cryo.md` for the other two).
...
All three actually written in `electric_production.py`: `calculate_acpow`,
...
`electric_production.py`. Switches (`i_pf_energy_storage_source`, `itart`,
