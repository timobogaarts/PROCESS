...
**Ported (2 units).** `tf_coil_power.py` / `test_tf_coil_power.py`:
...
`thermal_cryo.md`/`electric_production.md` for the other two --
...
`physics/{pure_formulas,composition,dimensionless_parameters}.md` used at comparable
size).
...
def calculate_tf_power_superconducting(
    ...
    ...
```
Both actually written in `tf_coil_power.py`.

## cottax node

`TfPowerResistive` and `TfPowerSuperconducting`, both `ExplicitFunction`, actually
written in `tf_coil_power.py`. Neither declares `.tfcoil.i_tf_sup` as a
...
- **`jnp.sqrt(n_tf_coils * res_tf_leg * 1000.0)` at `res_tf_leg = 0.0`** (`minor`,
  ...
  category `pure_formulas.md` names): the *value* at `res_tf_leg = 0.0`
  ...
  finding #2) -- `test_tf_coil_power.py`'s fuzz bounds deliberately exclude
