...
**Ported (5 units).** `thermal_cryo.py` / `test_thermal_cryo.py`:
...
`tf_coil_power.md`/`electric_production.md` for the other two).
...
(`thermal_cryo.py`); this table covers only the findings that shaped the
...
At the node level, `thermal_cryo.py` now declares this self-reference as a
...
`calculate_delta_eta`'s and `DeltaEtaStep`'s docstrings in `thermal_cryo.py`,
...
1. **`ElectricConversionModelTypes.SUPERCRITICAL_CO2_CYCLE` does not exist**
   ...
   against PROCESS -- excluded from `test_thermal_cryo.py`'s fuzz combos, see
   open questions.
...
All five actually written in `thermal_cryo.py`:
`calculate_plant_thermal_efficiency`, `calculate_plant_thermal_efficiency_2`,
`calculate_component_thermal_powers`, `calculate_cryo`, `calculate_cryo_loads` --
...
`thermal_cryo.py`. Every switch (`i_thermal_electric_conversion`,
...
`FastAlphaBeta` precedent (`pure_formulas.py`) -- none is wrapped in
...
None of these are yet in `_audit/units/core/solver/switches.md` (out of this fork's edit
...
`pure_formulas.md`'s denominator guards describe).
