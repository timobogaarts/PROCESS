...
Unit #23's audit record and port
(`functional_process/_audit/units/models/physics/impurity_radiation.md`/`.py`)
...
`composition.py` and the index table below). Both functions are ported and
...
(`test_composition.py::test_plasma_composition_owns_h_and_he_fractions`).
...
- **Denominator-guard-required** (same category flagged in `pure_formulas.md`
