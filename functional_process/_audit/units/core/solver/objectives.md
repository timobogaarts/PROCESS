...
| 4 | `P_TF_PLUS_P_PF` | `.tfcoil.tfcmw`, `.pf_power.srcktpm` | `tfcmw`: yes (`tf_coil_power.py`). `srcktpm`: **yes since 2026-08-30** (`models/power/pf_coil_power.py::PfCoilPowerSupplies`, on a tokamak) | `srcktpm` (PF coil circuit power) was a real hole, not a design-variable case like `rmajor`: `Power.pfpwr` was unported and the field was frozen at `0.0`. It is ported now, so this objective is computable from a tokamak's graph. It stays a hole on a *stellarator*, and correctly so — `stellarator.py` never calls `Power.run`, PROCESS leaves `srcktpm` at `0.0` there, and the port's `Power.pf_coil_power` slot is `None`. |
