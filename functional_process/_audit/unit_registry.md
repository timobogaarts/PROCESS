...
| 7 | `neoclassics.py` | 841 | `functional_process/models/stellarator/neoclassics.md` | **draft — no chunking needed** (13 clean sub-methods, no line-range splits required, one record). Dispatched specifically to hunt for a self-contained tier-2 unit — **none found**: despite the size, `init_neoclassics`/`calc_neoclassics` are straight-line tier-3 pipelines (each sub-method called exactly once, fixed order), not an internal solve. 2 of 13 pure functions ported + harness-tested (`calculate_profile_values`, `calculate_effective_thermal_diffusivity`, both fully scalar-argument, both `ExplicitFunction` nodes registered); the other 10 are ported (faithful, tier-1) but **not yet harness-tested** — flagged as a genuine harness gap, not a property of these functions: `Tier1Contract`'s gradient check assumes every differentiated argument is scalar, and these take species-length-4 or quadrature-length-30 array arguments. Expected to recur (fusion_reactions.py/radiation_power.py are candidates) — worth fixing once in `_harness/contracts.py` rather than per-unit. Also found: `.neoclassics.iota` and `.neoclassics.er` are read but never written anywhere in this file (external producer not yet located — candidates: `st_geom`/1C for `iota`, unknown for `er`); `iota` is confusingly two different things under one name within this same file (a `data` field vs. a forwarded argument, not asserted equal). |
...
| 12 | `physics/plasma_profiles.py` | `PlasmaProfile.run()` | `functional_process/models/physics/plasma_profiles.md` | pending |
...
Every ported node is registered in `functional_process/total_process.py`
(`to_graph(...)`) — running `python -m functional_process.render_xdsm` draws the current
graph to `functional_process/xdsm.html` for visual inspection. Both are meant to be kept
current as units land, not rebuilt from scratch each time.
