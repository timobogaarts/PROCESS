# Next steps — archive of sections 1–25

Verbatim bodies of `next_steps.md` §1–§25, moved here 2026-09-01 when the live file
reached 6257 lines. **Nothing is renumbered and nothing is deleted**: every section
keeps its number, and `next_steps.md` keeps a stub for each pointing here, so every
citation from a `.py` docstring or another `_audit` record still resolves. Measurements
here were true when taken; re-measure before relying on one.
