...
from functional_process.configurations import Configuration
from functional_process.cottax.input.importer import Imported, read_indat
...
from functional_process.cottax.models.blankets.hcpb import (
...
from functional_process.cottax.models.fw import (
    FirstWallDoubleNull,
    FirstWallDShapedDoubleNull,
    FirstWallSingleNull,
)
from functional_process.cottax.models.initialisation import (
...
from functional_process.cottax.models.pfcoil.namespace import (
    CSCoil,
    PFCoil,
    PFCoilCsWstNb3Sn,
...
from functional_process.cottax.models.physics.namespace import (
    Physics,
...
from functional_process.cottax.models.physics.plasma_profiles import lmode_profile_reset
...
from functional_process.cottax.models.stellarator.density_limits import EcrhDensityLimit
from functional_process.cottax.models.stellarator.heating import (
    EcrhHeating,
    LowhybHeating,
)
from functional_process.cottax.models.stellarator.namespace import (
...
from functional_process.cottax.models.tfcoil.base import (
...
from functional_process.cottax.models.total_process import (
    StellaratorProcess,
    TokamakProcess,
)
...
"""The run this whole port was first validated against."""
...
def machine_from_indat(input_file, stella_conf=None):
    """The `StellaratorProcess`/`TokamakProcess` an IN.DAT describes."""
    imported = _as_imported(input_file)
...
def configuration_from_indat(input_file, stella_conf=None) -> Configuration:
    """The `Configuration` an IN.DAT describes: its machine, its own values, its
    problem. The converter -- `configurations/<stem>.py` is this, written out.

    Raises
    ------
    ValueError
        If the file assigns an array no dataclass default gives a shape to.
    """
    from functional_process.cottax.input import native  # noqa: PLC0415

    imported = _as_imported(input_file)
    values, unshapeable = native.stated_values(imported)
    if unshapeable:
        raise ValueError(
            f"{Path(imported.path).name} assigns {unshapeable}, arrays no dataclass "
            f"default gives a shape to -- add the default to "
            f"`configurations/defaults.py`"
        )
    switches = switch_values_from_indat(imported)
    return Configuration(
        name=native.stem(imported.path),
        machine=machine_from_indat(imported, stella_conf),
        values=values,
        problem=native.problem_from_indat(imported, switches),
    )


def write_configuration(input_file, out=None, stella_conf=None) -> Path:
    """Convert `input_file` and write `configurations/<stem>.py` (or `out`)."""
    from functional_process.configurations import spell  # noqa: PLC0415
    from functional_process.cottax.input import native  # noqa: PLC0415

    configuration = configuration_from_indat(input_file, stella_conf)
    if out is None:
        out = (
            Path(__file__).resolve().parents[2]
            / "configurations"
            / f"{native.stem(str(input_file))}.py"
        )
    source = Path(str(input_file))
    source = str(source.relative_to(Path(__file__).resolve().parents[3])) if source.is_absolute() else str(source)
    out.write_text(
        spell(
            configuration,
            source=source,
            context={"stella_conf": str(REFERENCE_STELLA_CONF.relative_to(Path(__file__).resolve().parents[3])) if stella_conf is None else str(stella_conf)},
        )
    )
    return out
