...
def prepare(config: str, max_entries: int | None = None) -> Prep:
    ...
    # `wp.array` global suffices instead of a per-thread `vec{n}f`. Computed once, per
    # config, by `_constant_varpaths` -- every node's `const_inputs` agrees.
    ...
    array_boundary = ([p for p in boundary if p in live_arrays and p not in live_const]
                       + [p for p in boundary if p in live_arrays and p in live_const])
    ...
    kernel_src, _mapper = build_kernel_source(
        ...
        array_vars=live_arrays, const_arrays=live_const)
