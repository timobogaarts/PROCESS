"""Block-by-block comparison: does `functional_process.cottax.mda`'s driven graph
reproduce a real, converged PROCESS run's own values?
"""
...
from functional_process.cottax.mda import driven_graph, given_start, starts_for
...
from functional_process.cottax.indat import STATED_VALUES  # noqa: E402
...
`NodePath` (`.stellarator.coils.intersect`, `.tokamak.build.tf_outboard_mid`).
"""
...
`device_root(graph)` says so.
"""

ANY_DEVICE = None
"""Guard value in `KNOWN_UNVERIFIABLE_OUTPUTS`: applies on every machine."""
...
SWITCH = "switch"
"""Kind (a), `_audit/switch_elimination_design.md` §3: a model-selection switch."""

SHAPE = "shape/resolution (kind b)"
"""Kind (b): an array shape or loop trip count."""
...
class SwitchMismatch:
    """One node's static switch kwarg whose registered value differs from the value the
    converged run actually used.
    """
    ...
    actual: object
    enum: type | None = None
    """The `IntEnum` this kwarg is typed with, when it has one."""
...
class SwitchAudit:
    ...
    `DataStructure` field *and* that are not declared non-switches.
    """
    ...
    `int`/`bool` rather than an `IntEnum` member.
    """


def _declaration_modules(obj, seen):
    """Every `functional_process`-defined `equinox.Module` reachable from one graph node
    definition -- the declaration instance itself plus anything it holds.
    """
    if id(obj) in seen:
        return
    seen.add(id(obj))
...
"""Where converged `DataStructure`s are cached."""
...
def converged_data(input_file: str, use_cache: bool = True):
    ...
    return the resulting live `DataStructure`.
    """
...
@dataclass
class Disagreement:
    """One `VarPath` where the schedule's answer and `data`'s own converged value don't
...
class ComparisonReport:
    ...
    """Boundary inputs / driven unknowns with no real `DataStructure` field to seed from
    -- the same shape as `DuctDiameterRootFind`'s exclusion (a minted local PROCESS
    never stores), just discovered empirically rather than pre-flagged.
    """
    unverifiable: list = field(default_factory=list)
    """Outputs that read an `ungrounded_inputs` entry, directly or transitively -- not a
    disagreement, since there is nothing real to disagree with.
    """
    switches: SwitchAudit = field(default_factory=SwitchAudit)
    """`switch_audit()`'s result -- every registered static switch kwarg checked against
    the value this run actually used.
    """
    owned_total: int = 0
    """How many owned variables `compare()` walked."""
    unaccounted: list = field(default_factory=list)
    """Owned variables that landed in no bucket at all."""
    array_agreements: int = 0
    """How many of `agreements` were array-valued (compared elementwise)."""
