...
from functional_process.cottax.input.indat import GRAPH, graph_for, machine_from_indat

OUTDIR = Path(__file__).resolve().parents[2]
"""`functional_process/`: where the rendered pages go (gitignored)."""
...
def main(input_file: str | None = None):
    ...
    graph itself, which a drawing takes since cottax `4d33cf3` -- an `ExecutableGraph`
...
def grouped(depth: int | None = None, input_file: str | None = None):
    ...
    blocking = ExecutableGraph(graph)
...
def grouped_uncut(depth: int | None = None, input_file: str | None = None):
    ...
    declared, suffix = machine_graph(input_file)
    # The graph itself, not `ExecutableGraph(declared)`: since cottax `bc1130a` a
...
"""The conventional tokamak `--machine` draws."""


def cold_reference(input_file=None):
    """What the SAND assembly needs, from the configuration alone -- **no solve**:
    the checked-in `stellarator_helias`, or `input_file` converted.
    """
    from functional_process.configurations import load  # noqa: PLC0415
    ...
    configuration = (
        load("stellarator_helias")
        if input_file is None
        else configuration_from_indat(input_file)
    )
    return native.reference_of(configuration)
...
def sand():
    ...
    for name, blocking in (("xdsm_sand", ExecutableGraph(combined)),):
        render_xdsm_html(
            blocking,
            file_name=name,
            outdir=str(OUTDIR),
...
USAGE = """usage: python -m functional_process.cottax.visualization.render_xdsm [sand|grouped|grouped_uncut]
                                                [--machine [IN.DAT]]
...
"""Spelled in the one long form, `.area.field`."""
