...
class AcpowLine(Acpow, WrapsFunction):
    """`i_pf_energy_storage_source == LINE` (2) -- the reference run's."""

    fn = calculate_acpow_line
...
class AcpowMotorGeneratorFlywheel(Acpow, WrapsFunction):
    ...
    fn = calculate_acpow_motor_generator_flywheel
...
class PowerProfilesOverTime(WrapsFunction):
    """cottax node: `power_profiles_over_time`."""

    fn = power_profiles_over_time
...
class PlantElectricProductionResistiveCentrepostLiquidBreeder(
    PlantElectricProductionReactor, WrapsFunction
):
    """Resistive centrepost and liquid breeder -- both extra reads at once."""

    fn = calculate_plant_electric_production_resistive_centrepost_liquid_breeder
