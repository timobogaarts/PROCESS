...
"""Five of the ten `FACTORY_READ_SWITCHES`: each has a second registered occupant that
...
_CAUSES_A_REFUSAL = (
    ...
    ("ipowerflow", 0, ("blktmodel_ipowerflow", 0)),
    ("blktmodel", 1, ("blktmodel_blkttype", 1)),
    ("blkttype", 1, ("blktmodel_blkttype", 1)),
)
"""The remaining five `FACTORY_READ_SWITCHES`, refused for two different reasons.
...
`ipowerflow`/`blktmodel`/`blkttype`: each feeds a *joint* dispatch (`.fwbs.blktmodel` x
`.heat_transport.ipowerflow`, and x `.fwbs.blkttype`) alongside a second switch, so
changing just one of the three -- holding the others at the reference file's own values
-- drives the joint key to a value `UNPORTED` records rather than to a second occupant.
...
def test_factory_switches_that_only_prove_themselves_by_a_refusal(
    ...
    """The three joint-dispatch switches from `_CAUSES_A_REFUSAL`: `REFERENCE_MACHINE`
    itself assembles without error, but overriding just `switch` (holding the other
    member of its joint key at the reference file's value) drives the joint key to a
    value `UNPORTED` documents as not-yet-ported -- so `machine_from_indat` raises
